...
public class NasstorageCamera {

    private static final Logger logger = LoggerFactory.getLogger(NasstorageCamera.class);


    public NasstorageFile nasstorageCameraCaptureImg(NasstorageProperties properties, Integer imgType, String deviceNo, String imageData, String imageUrl, Long captureTime) {
        ...
        try {
            ...
            String faceDir = EnumClass.StorageImageType.STORAGE_IMAGE_TYPE_FACEPIC.getType();
            if (null != imgType && 2 == imgType.intValue()) {
                faceDir = EnumClass.StorageImageType.STORAGE_IMAGE_TYPE_FULLPIC.getType();
            }
            NasstorageNode node = (new NasstorageNodeChoice()).getNasstorageNode(properties);
            nasstorageFile = (new NasstorageFileBuilder()).nasstorageCameraFile(node, faceDir, deviceNo, fileName, captureTime);
