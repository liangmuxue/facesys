...
public class NasstorageWarning {

    private static final Logger logger = LoggerFactory.getLogger(NasstorageWarning.class);

    public NasstorageFile nasstorageWarningImg(NasstorageProperties properties, String imageData, String imageUrl, Long warningTime) {
        ...
        try {
            ...
            String rootDir = EnumClass.StorageImageType.STORAGE_IMAGE_TYPE_WARNING.getType();
            NasstorageNode node = (new NasstorageNodeChoice()).getNasstorageNode(properties);
            nasstorageFile = (new NasstorageFileBuilder()).nasstorageWarningFile(node, rootDir, filename, warningTime);
