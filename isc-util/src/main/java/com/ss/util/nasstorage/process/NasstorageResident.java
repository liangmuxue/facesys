...
public class NasstorageResident {

    private static final Logger logger = LoggerFactory.getLogger(NasstorageResident.class);

    public NasstorageFile nasstorageResidentImg(NasstorageProperties properties, String buildingCode, String imageData, String imageUrl) {
        ...
        try {
            ...
            String rootDir = EnumClass.StorageImageType.STORAGE_IMAGE_TYPE_RESIDENT.getType();
            NasstorageNode node = (new NasstorageNodeChoice()).getNasstorageNode(properties);
            nasstorageFile = (new NasstorageFileBuilder()).nasstorageResidentFile(node, rootDir, buildingCode, filename);
