...
public class NasstorageUser {

    private static final Logger logger = LoggerFactory.getLogger(NasstorageUser.class);


    public NasstorageFile nasstorageUserImg(NasstorageProperties properties, String imageData, String imageUrl) {
        ...
        try {
            ...
            String rootDir = EnumClass.StorageImageType.STORAGE_IMAGE_TYPE_USER.getType();
            NasstorageNode node = (new NasstorageNodeChoice()).getNasstorageNode(properties);
            nasstorageFile = (new NasstorageFileBuilder()).nasstorageUserFile(node, rootDir, filename);
