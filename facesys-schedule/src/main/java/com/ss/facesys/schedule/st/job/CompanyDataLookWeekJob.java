 ...
 **/
public class CompanyDataLookWeekJob implements SimpleJob {

    public static final Log LOG = LogFactory.getLog(CompanyDataLookWeekJob.class);
    private IdataLookService idataLookService = SpringUtil.getBean(IdataLookService.class);

    @Override
    public void execute(ShardingContext shardingContext) {
        LOG.info("定时任务HouseDataLookWeekJob已经启动" + DateUtil.getCurrentSqlTimestampString());
        //本周数据
        int n =1;//n周前 0本周
        for(int weekNum=1;weekNum <= n;weekNum++){
            //本月数据
            this.idataLookService.selThisWeekCompany(weekNum);
        }
    }
}
