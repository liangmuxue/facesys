 ...
 **/
public class PerceptDataLookDayJob implements SimpleJob {

    public static final Log LOG = LogFactory.getLog(PerceptDataLookDayJob.class);
    private IdataLookService idataLookService = SpringUtil.getBean(IdataLookService.class);

    @Override
    public void execute(ShardingContext shardingContext) {
        LOG.info("定时任务PerceptDataLookDayJob已经启动" + DateUtil.getCurrentSqlTimestampString());
        int n=1;//n天前
        for(int dayNum=1;dayNum <= n;dayNum++){
            //每天数据(近30天)
            this.idataLookService.selThisThirtyDayPercept(dayNum);
        }
