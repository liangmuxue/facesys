 ...
 **/
public class CompanyDataLookMouthJob implements SimpleJob {

    public static final Log LOG = LogFactory.getLog(CompanyDataLookMouthJob.class);
    private IdataLookService idataLookService = SpringUtil.getBean(IdataLookService.class);

    @Override
    public void execute(ShardingContext shardingContext) {
        LOG.info("定时任务HouseDataLookMouthJob已经启动" + DateUtil.getCurrentSqlTimestampString());
        int n = 1;
        for(int mouthNum = 1;mouthNum <= n;mouthNum++){
            //本月数据
            this.idataLookService.selthisMouthCompany(mouthNum);
        }
    }
}
