...
public class NasstorageDeviceTerminal {

    private static final Logger logger = LoggerFactory.getLogger(NasstorageDeviceTerminal.class);


    public NasstorageFile nasstorageDeviceTerminalImg(NasstorageProperties properties, Integer imgType, String deviceNo, String imageData, String imageUrl, Long captureTime) {
        ...
        try {
            ...
            String faceDir = EnumClass.StorageImageType.STORAGE_IMAGE_TYPE_DT_FACE.getType();
            if (null != imgType && 2 == imgType.intValue()) {
                faceDir = EnumClass.StorageImageType.STORAGE_IMAGE_TYPE_DT_DTFACEFULL.getType();
            }
            NasstorageNode node = (new NasstorageNodeChoice()).getNasstorageNode(properties);
            nasstorageFile = (new NasstorageFileBuilder()).nasstorageDeviceTerminalFile(node, faceDir, deviceNo, fileName, captureTime);
