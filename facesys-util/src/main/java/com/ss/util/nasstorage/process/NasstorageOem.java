...
public class NasstorageOem {

    private static final Logger logger = LoggerFactory.getLogger(NasstorageVictory.class);


    public NasstorageFile nasstorageOemImg(NasstorageProperties properties, String imageData, String imageUrl, Long oemTime) {
        ...
        try {
            ...
            String rootDir = EnumClass.StorageImageType.STORAGE_IMAGE_TYPE_OEM.getType();
            NasstorageNode node = (new NasstorageNodeChoice()).getNasstorageNode(properties);
            nasstorageFile = (new NasstorageFileBuilder()).nasstorageOemFile(node, rootDir, filename, oemTime);
