...
public class NasstorageVictory {

    private static final Logger logger = LoggerFactory.getLogger(NasstorageVictory.class);


    public NasstorageFile nasstorageVictoryImg(NasstorageProperties properties, String imageData, String imageUrl, Long victoryTime) {
        ...
        try {
            ...
            String rootDir = EnumClass.StorageImageType.STORAGE_IMAGE_TYPE_VICTORY.getType();
            NasstorageNode node = (new NasstorageNodeChoice()).getNasstorageNode(properties);
            nasstorageFile = (new NasstorageFileBuilder()).nasstorageWarningFile(node, rootDir, filename, victoryTime);
