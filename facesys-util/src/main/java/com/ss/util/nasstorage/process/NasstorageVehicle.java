...
public class NasstorageVehicle {

    private static final Logger logger = LoggerFactory.getLogger(NasstorageVehicle.class);


    public NasstorageFile nasstorageVehicleCaptureImg(NasstorageProperties properties, Integer imgType, String deviceNo, String imageData, String imageUrl, Long captureTime) {
        ...
        try {
            ...
            String faceDir = EnumClass.StorageImageType.STORAGE_IMAGE_TYPE_CARNOPIC.getType();
            if (null != imgType && 2 == imgType.intValue()) {
                faceDir = EnumClass.StorageImageType.STORAGE_IMAGE_TYPE_CARFULLPIC.getType();
            }
            NasstorageNode node = (new NasstorageNodeChoice()).getNasstorageNode(properties);
            nasstorageFile = (new NasstorageFileBuilder()).nasstorageVehicleCaptureFile(node, faceDir, deviceNo, fileName, captureTime);
