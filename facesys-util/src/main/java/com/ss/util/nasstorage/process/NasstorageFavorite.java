...
public class NasstorageFavorite {

    private static final Logger logger = LoggerFactory.getLogger(NasstorageFavorite.class);


    public NasstorageFile nasstorageFavoriteImg(NasstorageProperties properties, String imageData, String imageUrl, Long favoriteTime) {
        ...
        try {
            ...
            String rootDir = EnumClass.StorageImageType.STORAGE_IMAGE_TYPE_FAVORITE.getType();
            NasstorageNode node = (new NasstorageNodeChoice()).getNasstorageNode(properties);
            nasstorageFile = (new NasstorageFileBuilder()).nasstorageWarningFile(node, rootDir, filename, favoriteTime);
