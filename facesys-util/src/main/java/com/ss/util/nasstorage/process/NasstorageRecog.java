...
public class NasstorageRecog {

    private static final Logger logger = LoggerFactory.getLogger(NasstorageRecog.class);


    public NasstorageFile nasstorageRecogImg(NasstorageProperties properties, String imageData, String imageUrl, Long recogTime) {
        ...
        try {
            ...
            String rootDir = EnumClass.StorageImageType.STORAGE_IMAGE_TYPE_RECOG.getType();
            NasstorageNode node = (new NasstorageNodeChoice()).getNasstorageNode(properties);
            nasstorageFile = (new NasstorageFileBuilder()).nasstorageRecogFile(node, rootDir, filename, recogTime);
