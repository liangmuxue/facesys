...
import com.ss.isc.data.baseinfo.common.dto.PersonCaptureDTO;
import com.ss.isc.data.collect.client.ICaptureService;
import com.ss.isc.data.collect.common.dto.CaptureCompareDTO;
import com.ss.isc.data.collect.common.dto.RegisterDTO;
import com.ss.isc.data.collect.common.model.Camera;
import com.ss.isc.data.collect.common.model.VehicleRecord;
import com.ss.isc.data.collect.common.web.CaptureCompareVO;
import com.ss.isc.data.collect.common.web.CaptureImgVO;
import com.ss.isc.data.collect.common.web.CaptureQueryVO;
 ...
 */
@RestController
@RequestMapping({"/collect/capture"})
public class CaptureController extends BaseController {
    ...
    public ResponseEntity<JSONObject> peers(@RequestBody @Validated({APIPageGroup.class}) CaptureQueryVO captureQueryVO, BindingResult bindingResult) throws Exception {
        ...
        try {
            ...
            if (flag) {
                ...
                if (StringUtils.checkSuccess(jsonObject)) {
                    ...
                    String deviceId = "";
                    com.ss.isc.data.resource.common.model.Camera device = null;
                    ...
                    //添加性别名称
                    for (int i = 0; i < datas.size(); i++) {
                        if (datas.getJSONObject(i).getString("gender") == null){
                            continue;
                        }
                        int gender = Integer.parseInt(datas.getJSONObject(i).getString("gender"));
                        if (CommonConstant.COMMON_1 == gender) {
                            datas.getJSONObject(i).put("genderName", Enums.Sex.getName(CommonConstant.COMMON_1));
                        } else if (CommonConstant.COMMON_2 == gender) {
                            datas.getJSONObject(i).put("genderName", Enums.Sex.getName(CommonConstant.COMMON_2));
                        } else {
                            datas.getJSONObject(i).put("genderName", Enums.Sex.getName(CommonConstant.COMMON_3));
                        }
                    }
        ...
        try {
            ...
            if (flag) {
                ...
                for (PersonCaptureDTO captureDTO : pList) {
                    ...
                    //添加性别名称
                    if (CommonConstant.COMMON_1.equals(captureDTO.getGender())){
                        captureDTO.setGenderName(Enums.Sex.getName(CommonConstant.COMMON_1));
                    } else if (CommonConstant.COMMON_2.equals(captureDTO.getGender())){
                        captureDTO.setGenderName(Enums.Sex.getName(CommonConstant.COMMON_2));
                    } else {
                        captureDTO.setGenderName(Enums.Sex.getName(CommonConstant.COMMON_3));
                    }
                }
                // 封装结果
